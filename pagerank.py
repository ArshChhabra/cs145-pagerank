...
class WebGraph():
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        with torch.no_grad():
            ...
            #Finding Non Dangling Nodes then Computing a
            nd_nodes = torch.sparse.sum(self.P,1).indices()
            a = torch.ones([n,1])
            a[nd_nodes] = 0
            ...
            for i in range(max_iterations):
                ...
                #Computing New x using PM and normlization
                vec = (alpha * x.t()@a+(1-alpha))*v.t()
                x = torch.sparse.addmm(vec.t(), self.P.t(),x,beta = 1,alpha = alpha)
                x /= torch.norm(x)
